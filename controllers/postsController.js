...
const postsController = {
    ...
    async createPost(req, res, next){
        ...
        const newPostDetail = await newPost.populate({ path: 'user',
                                                       select: ' name photo ' });
        const result = { post: newPostDetail };
        successHandle(res,result);
    },
    ...
    async deletePostComment(req, res, next){
        ...
        const deleteCommentPost = await Post.findById(deleteComment.post).populate({
            path: 'user',
            select: 'id'
        });
